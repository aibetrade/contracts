...
contract TarifReader {
    ...
    function isRegister(uint256 _tarif) public pure returns (bool) {
        return TarifReaderLib.tarifKey(_tarif) == REGISTRATION_KEY;
    }

    function isPartner(uint256 _tarif) public pure returns (bool) {
        return TarifReaderLib.getNumSlots(_tarif) > 0;
    }
    ...
    function hasSlot(uint256 _tarif, uint64 _usage) public pure returns (bool) {
        return
            getUsedSlots(_usage) <
            TarifReaderLib.getNumSlots(_tarif) * (getExtLevel(_usage) + 1);
    }
    ...
    function hasLVSlot(
        ...
        return
            getUsedLVSlots(_usage) <
            TarifReaderLib.getNumLVSlots(_tarif) * (getExtLevel(_usage) + 1);
    }
...
contract TarifsContractBase is OnlyOwner {
    ...
    function tarif(uint16 _key) public view returns (uint256) {
        for (uint8 i = 0; i < tarifs.length; i++)
            if (TarifReaderLib.tarifKey(tarifs[i]) == _key) return tarifs[i];
        return 0;
    }
    ...
    function exists(uint256 _tarif) public view returns (bool) {
        uint16 key = TarifReaderLib.tarifKey(_tarif);
        for (uint8 i = 0; i < tarifs.length; i++) {
            if (TarifReaderLib.tarifKey(tarifs[i]) == key) return true;
        }
        return false;
    }
    ...
    function append(uint256 _tarif) public onlyOwner {
        require(
            !exists(_tarif) &&
                !TarifReaderLib.getIsRejected(_tarif) &&
                !TarifReaderLib.getIsComsaTaken(_tarif)
        );
        tarifs.push(_tarif);
    }
    ...
    function isLast(uint256 _tarif) public view returns (bool) {
        ...
        return TarifReaderLib.tarifKey(_tarif) == TarifReaderLib.tarifKey(tarifs[tarifs.length - 1]);
    }
}
...
contract TarifsContract is TarifReader {
    ...
    function isT1BetterOrSameT2(
        ...
        bool t2Found = false;
        uint16 k1 = TarifReaderLib.tarifKey(_tarif1);
        uint16 k2 = TarifReaderLib.tarifKey(_tarif2);

        if (k2 == 0) return true; // Any model better then none.
        // if (k1 == k2) return true;

        for (uint8 i = 0; i < partnerTarifs.tarifsCount(); i++) {
            if (TarifReaderLib.tarifKey(partnerTarifs.tarifs(i)) == k2) t2Found = true;
            if (TarifReaderLib.tarifKey(partnerTarifs.tarifs(i)) == k1) return t2Found;
        }
