...
import "../stores/TarifsStore.sol";
import "../stores/UsersTreeStore.sol";
import "../apps/UsersFinanceApp.sol";
import "../stores/RankMatrix.sol";
...
contract Referal is BaseApp {
    ...
    function setCWallet(address _cWallet) public onlyMember {
        require(_cWallet != address(0));
        cWallet = _cWallet;
    }

    address public qcWallet;
    function setQCWallet(address _qcWallet) public onlyMember {
        require(_qcWallet != address(0));
        qcWallet = _qcWallet;
    }

    address public qpWallet;
    function setQPWallet(address _qpWallet) public onlyMember {
        require(_qpWallet != address(0));
        qpWallet = _qpWallet;
    }

    address public mWallet;
    function setMWallet(address _mWallet) public onlyMember {
        require(_mWallet != address(0));
        mWallet = _mWallet;
    }

    uint16 public qBonus;
    function setQBonus(uint8 _qBonus) public onlyMember {
        require(_qBonus < 101);
        qBonus = _qBonus;
    }

    uint16 public registerPrice;
    function setRegisterPrice(uint16 _registerPrice) public onlyMember {
    ...
    function canTakeComsa(address _client) public view returns(bool){      
        ...
        BuyHistoryRec memory buy = buyHistory.getLast(_client);
        if (buy.tarif == 0 || buy.state == BUY_STATE_REJECTED || block.timestamp - buy.timestamp < 48 * 3600) return false;        

        AddressFlagsStore comsas = appStore.apps(COMSAS_STORE_CODE);
        return comsas.flasg(_client);
    }
    ...
    function clientScheme(address _client, address mentor, uint32 curPriceCent) internal returns(uint32){
        UsersTreeStore usersTree = appStore.apps(USERS_TREE_CODE);
        UsersTarifsStore usersTarifsStore = appStore.apps(USERS_TARIFS_CODE);
        UsersFinanceApp usersFinance = appStore.apps(USERS_FINANCE_APP_CODE);
